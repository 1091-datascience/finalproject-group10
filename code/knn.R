...
for(i in 1:k_fold){
  ...
  tmp_model <- train.kknn(as.factor(lab) ~ ., train_set, kmax = kv, kernel = c("triangular", "rectangular", "epanechnikov", "optimal"), distance = 2)
  
  train_results <- confusionMatrix(table(pred=predict(tmp_model, train_set), train_set$lab))
  
  test_results <- confusionMatrix(table(pred=predict(tmp_model, test_set), test_set$lab))
